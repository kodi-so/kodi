 ...
 *
 * Why: navigating between pages (dashboard → settings → chat) loses any
...
export function OrgProvider({ children }: { children: ReactNode }) {
  ...
  const applyOrgs = useCallback((list: OrgOption[], preferredId?: string | null) => {
    setOrgs(list)
    const storedId = preferredId ?? (typeof window !== 'undefined' ? localStorage.getItem(LS_KEY) : null)
    const match = list.find(o => o.orgId === storedId) ?? list[0] ?? null
    setActiveOrgState(match)
    if (match && typeof window !== 'undefined') localStorage.setItem(LS_KEY, match.orgId)
  }, [])
